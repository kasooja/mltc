	...
	public static void main(String[] args) throws Exception {	// Option Parser
		...
		MultiLayerConfiguration.Builder builder = new NeuralNetConfiguration.Builder()
				...
				.layer(0, new ConvolutionLayer.Builder(typeOfFilter, vectorSize)
						...
						.activation("relu")
						.build())
				.layer(1, new GravesLSTM.Builder().nIn(noOfFilters).nOut(hiddenLayerSize)
		...
		for(int i=0; i<nEpochs; i++){
			...
			if(i%evalEveryN == 0){
				...
				while(test.hasNext()){
					...
					//INDArray tensorAlongDimension = predicted.tensorAlongDimension(0, 0, 0);

					INDArray pred = Nd4j.create(t.numExamples(), 2);

					for(int m=0; m<t.numExamples(); m++){
						INDArray indArray = predicted.get(new INDArrayIndex[]{NDArrayIndex.point(m), NDArrayIndex.all(), NDArrayIndex.point(0)});
						INDArray reshape = indArray.reshape(1, 2);
						pred.putRow(m, reshape);
					}	
					//evaluation.evalTimeSeries(labels, pred);//, outMask);
					evaluation.eval(labels, pred);
